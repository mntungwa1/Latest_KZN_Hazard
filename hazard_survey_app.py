...
def display_map():
    ...
    if TOPOJSON_PATH.exists():
        ...
        folium.TopoJson(
            ...
            name="Wards",
            tooltip=folium.GeoJsonTooltip(fields=["UID"], aliases=["Ward:"])
        ).add_to(m)
    ...
    return st_folium(m, height=700, width=1200)
...
def run_survey():
    ...
    ward_display = clicked_ward.get("properties", {}).get("UID") if clicked_ward else ""
    if ward_display:
        st.session_state["selected_ward"] = ward_display
        st.success(f"Selected Ward: {ward_display}")

    st.subheader("Select Applicable Hazards")
    selected = st.multiselect("Choose hazards:", hazards, key="hazard_select")
    custom = st.text_input("Other hazard", key="custom_hazard") if st.checkbox("Add custom hazard") else ""
